...
export default function CampaignDashboardClient() {
  ...
  if (!campaign) return (
    <div role="status" aria-label="Loading" className="flex items-center gap-3 text-stone-500 py-16 justify-center">
      <span className="text-2xl animate-pulse" aria-hidden="true">⚔</span>
      <span>Loading campaign…</span>
    </div>
  );

  const npcCount     = campaign.npcCount     ?? 0;
  const sessionCount = campaign.sessionCount ?? 0;
  const lastPlayedOn = campaign.lastPlayedOn ?? null;
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <StatCard
          icon="🗓"
          label="Last Played"
          value={lastPlayedOn ? new Date(lastPlayedOn).toLocaleDateString() : "—"}
          accent="green"
        />
      </div>
