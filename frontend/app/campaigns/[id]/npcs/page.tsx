...
export default function NpcRosterPage() {
  ...
  async function handleDelete(npc: Npc) {
    if (!confirm(`Remove ${npc.name}? This cannot be undone.`)) return;
    try {
  ...
  const filteredNpcs = npcs.filter((npc) => {
    if (filterStatus === "alive" && !npc.isAlive) return false;
    if (filterStatus === "deceased" && npc.isAlive) return false;
    if (filterRelationship !== "all" && npc.relationshipToParty !== filterRelationship) return false;
    if (search.trim()) {
    ...
    <div>
      ...
      <div className="flex gap-5">
        ...
        <div className="w-60 shrink-0 space-y-2">
          ...
          {!loading && npcs.length > 0 && (
            <p className="text-stone-700 text-xs px-1">
          ...
          {/* NPC list */}
          {loading ? (
            <p className="text-stone-600 text-sm py-2">Loading…</p>
          ) : npcs.length === 0 ? (
            ...
            filteredNpcs.map((npc) => (
              <button
                key={npc.id}
                onClick={() => { setSelected(npc); setEditingNpc(false); setNpcDraft({}); }}
              ...
              <>
                <div className="flex items-start justify-between mb-5 gap-4">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-12 h-12 rounded-full bg-amber-900/30 border border-amber-900/40 flex items-center justify-center shrink-0">
      ...
      {showCreate && (
        ...
        >
          <form
            action={handleCreate}
            className="bg-stone-900 border border-stone-700/80 rounded-2xl p-6 w-full max-w-lg shadow-2xl shadow-black/70 space-y-3 max-h-[90vh] overflow-y-auto"
          >
            <div className="flex items-center justify-between mb-1">
              <h2 className="font-cinzel text-xl text-amber-400">Add NPC</h2>
              <button type="button" onClick={() => setShowCreate(false)} className="text-stone-600 hover:text-stone-300 text-lg transition-colors">✕</button>
            </div>
